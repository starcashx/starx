...
void ThreadSocketHandler()
{
    ...
    while (true)
    {
        ...
        if (hListenSocket != INVALID_SOCKET && FD_ISSET(hListenSocket, &fdsetRecv))
        {
            ...
            {
                // STARXrding to the internet TCP_NODELAY is not carried into accepted sockets
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "StarCashX " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. StarCashX is probably already running."), addrBind.ToString());
        else
