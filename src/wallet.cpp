...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CStarCashXcoinAddress(redeemScript.GetID()).ToString();
...
void CWallet::SyncMetaData(pair<TxSpends::iterator, TxSpends::iterator> range)
{
    ...
    for (TxSpends::iterator it = range.first; it != range.second; ++it)
    {
        ...
        copyTo->strFromSTARXunt = copyFrom->strFromSTARXunt;
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFromLoadWallet)
{
    ...
    if (fFromLoadWallet)
    {
        ...
        wtx.BindWallet(this);
        wtxOrdered.insert(make_pair(wtx.nOrderPos, TxPair(&wtx, (CSTARXuntingEntry*)0)));
        AddToSpends(hash);
    }
    else
    {
...
void CWalletTx::GetAmounts(list<pair<CTxDestination, int64_t> >& listReceived,
                           list<pair<CTxDestination, int64_t> >& listSent, CAmount& nFee, string& strSentSTARXunt, const isminefilter& filter) const
{
    LOCK(pwallet->cs_wallet);
    nFee = 0;
    listReceived.clear();
    listSent.clear();
    strSentSTARXunt = strFromSTARXunt;
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        // STARXNOTE: CoinControl possible fix related... with HD wallet we need to report change?
...
void CWalletTx::GetSTARXuntAmounts(const string& strSTARXunt, CAmount& nReceived,
                                  CAmount& nSent, CAmount& nFee, const isminefilter& filter) const
{
    ...
    GetAmounts(listReceived, listSent, allFee, strSentSTARXunt, filter);

    if (strSTARXunt == strSentSTARXunt)
    {
    ...
    }
    {
        ...
        BOOST_FOREACH(const PAIRTYPE(CTxDestination,int64_t)& r, listReceived)
        {
            if (pwallet->mapAddressBook.count(r.first))
            {
                ...
                if (mi != pwallet->mapAddressBook.end() && (*mi).second == strSTARXunt)
                    nReceived += r.second;
            }
            else if (strSTARXunt.empty())
            {
                nReceived += r.second;
            }
        }
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s STARX %s\n", FormatMoney(wtx.GetCredit(ISMINE_ALL)), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, AvailableCoinsType coin_type, bool useIX) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (nDepth <= 0) // STARXNOTE: coincontrol fix / ignore 0 confirm
                continue;
...
bool CWallet::SelectCoins(int64_t nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1STARX
    if(coin_type == ONLY_DENOMINATED) {
        ...
        BOOST_FOREACH(int64_t v, darkSendDenominations)
        {
            BOOST_FOREACH(const COutput& out, vCoins)
            {
                if(out.tx->vout[out.i].nValue == v                                            //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1*COIN)+100 //round the amount up to .1STARX over
                ){
                    CTxIn vin = CTxIn(out.tx->GetHash(),out.i);
                    int rounds = GetInputDarksendRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nDarksendRoundsMin, int nDarksendRoundsMax)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            ...
            //
            // bit 0 - 1000STARX+1 ( bit on if present )
            // bit 1 - 100STARX+1
            // bit 2 - 10STARX+1
            // bit 3 - 1STARX+1
            // bit 4 - .1STARX+1
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl, coin_type, useIX))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _(" Unable to locate enough Darksend non-denominated funds for this transaction that are not equal 1000 STARX.");
                    } else {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-StarCashX-address
                    CScript scriptChange;
...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        CStarCashXcoinAddress addr(ckid);
        ...
        if (fDebug)
        {
            ...
            CStarCashXcoinAddress coinAddress(keyID);
...
bool CWallet::SendStealthMoneyToDestination(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    CStarCashXcoinAddress addrTo(ckidTo);
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    if(hasPayment && payeerewardpercent == 0){
        ...
        CStarCashXcoinAddress address2(address1);
    ...
    if(hasPayment && payeerewardpercent > 0 && payeerewardpercent < 100){
        ...
        CStarCashXcoinAddress address4(address3);
...
bool CWallet::AddSTARXuntingEntry(const CSTARXuntingEntry& acentry, CWalletDB & pwalletdb)
{
    if (!pwalletdb.WriteSTARXuntingEntry_Backend(acentry))
        return false;

    laccentries.push_back(acentry);
    CSTARXuntingEntry & entry = laccentries.back();
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CStarCashXcoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).EraseName(CStarCashXcoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CStarCashXcoinAddress(address).ToString());
}
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentCoins found lost coin %s STARX %s[%d], %s\n",
            ...
            {
                LogPrintf("FixSpentCoins found spent coin %s STARX %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
