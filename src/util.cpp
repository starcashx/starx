...
int nAnonymizeStarCashXAmount = 1000;
...
#else
    const char* pszModule = "StarCashX";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\StarCashX
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\StarCashX
    // Mac: ~/Library/Application Support/StarCashX
    // Unix: ~/.StarCashX
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "StarCashX";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "StarCashX.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "StarCashXd.pid"));
