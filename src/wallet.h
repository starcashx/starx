...
public:
    ...
    std::list<CSTARXuntingEntry> laccentries;

    typedef std::pair<CWalletTx*, CSTARXuntingEntry*> TxPair;
    ...
    bool AddSTARXuntingEntry(const CSTARXuntingEntry&, CWalletDB & pwalletdb);
...
public:
    ...
    (
        ...
        if (!fRead)
        {
            pthis->mapValue["fromaccount"] = pthis->strFromSTARXunt;
        ...
        if (fRead)
        {
            pthis->strFromSTARXunt = pthis->mapValue["fromaccount"];
    ...
    void GetAmounts(std::list<std::pair<CTxDestination, int64_t> >& listReceived,
                    std::list<std::pair<CTxDestination, int64_t> >& listSent, CAmount& nFee, std::string& strSentSTARXunt, const isminefilter& filter) const;

    void GetSTARXuntAmounts(const std::string& strSTARXunt, CAmount& nReceived,
...
{
public:
    std::string strSTARXunt;
    int64_t nCreditDebit;
    int64_t nTime;
    std::string strOtherSTARXunt;
    std::string strComment;
    mapValue_t mapValue;
    int64_t nOrderPos;  // position in ordered transaction list
    uint64_t nEntryNo;
    ...
    IMPLEMENT_SERIALIZE
    (
        CSTARXuntingEntry& me = *const_cast<CSTARXuntingEntry*>(this);
        if (!(nType & SER_GETHASH))
            READWRITE(nVersion);
        // Note: strSTARXunt is serialized as part of the key, not here.
        READWRITE(nCreditDebit);
        READWRITE(nTime);
        READWRITE(strOtherSTARXunt);
