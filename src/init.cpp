...
void Shutdown()
{
    ...
    RenameThread("StarCashX-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: StarCashX.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: StarCashXd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 21698)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeStarCashXamount=<n> " + _("Keep N StarCashX anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. StarCashX is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. StarCashX is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("StarCashX version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "StarCashX server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of StarCashX") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart StarCashX to complete") << "\n";
    ...
    nAnonymizeStarCashXAmount = GetArg("-anonymizeStarCashXamount", 0);
    if(nAnonymizeStarCashXAmount > 999999) nAnonymizeStarCashXAmount = 999999;
    if(nAnonymizeStarCashXAmount < 2) nAnonymizeStarCashXAmount = 2;
    ...
    LogPrintf("Anonymize StarCashX Amount %d\n", nAnonymizeStarCashXAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1STARX+1000 == (.1STARX+100)*10
       10STARX+10000 == (1STARX+1000)*10
    */
