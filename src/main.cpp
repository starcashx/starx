...
unsigned int nStakeMinAge = 17 * 60 * 60; // 17 hour
...
const string strMessageMagic = "StarCashX Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	...
	else if (nHeight > 1 && nHeight < 9031)
		{
			nSubsidy = 2.54 * COIN;
		}
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
	...
	if (pindexBest->nHeight+1 > 7985357)
		{
			nSubsidy = STATIC_POS_REWARD /2; 
		}
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (pindexPrevPrev->pprev == NULL)
        return bnTargetLimit.GetCompact(); // premine to be easy
...
{
    RenameThread("StarCashX-loadblk");
...
{
    int64_t ret = blockValue * 1/1.53; //65%
