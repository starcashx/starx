...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeStarCashXAmount"))
        settings.setValue("nAnonymizeStarCashXAmount", 1000);
    nAnonymizeStarCashXAmount = settings.value("nAnonymizeStarCashXAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeStarCashXAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeStarCashXAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeStarCashXAmount:
            return QVariant(nAnonymizeStarCashXAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeStarCashXAmount:
            nAnonymizeStarCashXAmount = value.toInt();
            settings.setValue("nAnonymizeStarCashXAmount", nAnonymizeStarCashXAmount);
            emit AnonymizeStarCashXAmountChanged(nAnonymizeStarCashXAmount);
            break;
        default:
            break;
        }
