...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("StarCashX"))
        return false;
...
{
    // Convert StarCashX:// to StarCashX:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("StarCashX://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "StarCashX:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "StarCashX.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for StarCashX.lnk
...
{
    return GetAutostartDir() / "StarCashX.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a StarCashX.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=StarCashX\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the StarCashX app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add StarCashX app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("StarCashX-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  StarCashX-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("StarCashX-Qt"));
