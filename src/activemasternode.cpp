...
//
// Bootup the masternode, look for a 7280 STARX input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in StarCashX.conf */
...
{
    CStarCashXcoinAddress address(collateralAddress);
