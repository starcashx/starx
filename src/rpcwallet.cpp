...
static void accountingDeprecationCheck()
{
    if (!GetBoolArg("-enableaccounts", false))
        throw runtime_error(
            "STARXunting API is deprecated and will be removed in future.\n"
...
string STARXuntFromValue(const Value& value)
{
    string strSTARXunt = value.get_str();
    if (strSTARXunt == "*")
...
CScript _createmultisig(const Array& params)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        CStarCashXcoinAddress address(ks);
...
Value createmultisig(const Array& params, bool fHelp)
{
    ...
    CStarCashXcoinAddress address(innerID);
...
Value getnewpubkey(const Array& params, bool fHelp)
{
    ...
    string strSTARXunt;
    if (params.size() > 0)
        strSTARXunt = STARXuntFromValue(params[0]);
    ...
    pwalletMain->SetAddressBookName(keyID, strSTARXunt);
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new StarCashX address for receiving payments.\n"
            "If 'account' is specified (recommended), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"StarCashXaddress\"    (string) The new StarCashX address\n"
            "\nExamples:\n"
    ...
    return CStarCashXcoinAddress(keyID).ToString();
}


CStarCashXcoinAddress GetSTARXuntAddress(string strSTARXunt, bool bForceNew=false)
{
    CWalletDB walletdb(pwalletMain->strWalletFile);

    CSTARXunt account;
    walletdb.ReadSTARXunt(strSTARXunt, account);
    ...
    if (!account.vchPubKey.IsValid() || bForceNew || bKeyUsed)
    {
        ...
        pwalletMain->SetAddressBookName(account.vchPubKey.GetID(), strSTARXunt);
        walletdb.WriteSTARXunt(strSTARXunt, account);
    }

    return CStarCashXcoinAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current StarCashX address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"StarCashXaddress\"   (string) The account StarCashX address\n"
            "\nExamples:\n"
    ...
    // Parse the account first so we don't generate a key if there's an error
    string strSTARXunt = STARXuntFromValue(params[0]);

    Value ret;

    ret = GetSTARXuntAddress(strSTARXunt).ToString();
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"StarCashXaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"StarCashXaddress\"  (string, required) The StarCashX address to be associated with an account.\n"
    ...
    CStarCashXcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid StarCashX address");


    string strSTARXunt;
    if (params.size() > 1)
        strSTARXunt = STARXuntFromValue(params[1]);
    ...
    if (IsMine(*pwalletMain, address.Get()))
    {
        ...
        {
            string strOldSTARXunt = pwalletMain->mapAddressBook[address.Get()];
            if (address == GetSTARXuntAddress(strOldSTARXunt))
                GetSTARXuntAddress(strOldSTARXunt, true);
        }
        pwalletMain->SetAddressBookName(address.Get(), strSTARXunt);
    }
    else
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"StarCashXaddress\"\n"
            "\nReturns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"StarCashXaddress\"  (string, required) The StarCashX address for account lookup.\n"
            "\nResult:\n"
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"StarCashXaddress\"  (string) a StarCashX address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CStarCashXcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CStarCashXcoinAddress& address = item.first;
        const string& strName = item.second;
        if (strName == strSTARXunt)
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress \"StarCashXaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSent an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
           "1. \"StarCashXaddress\"  (string, required) The StarCashX address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in STARX to send. eg 0.1\n"
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"StarCashXaddress\",     (string) The StarCashX address\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CStarCashXcoinAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CStarCashXcoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CStarCashXcoinAddress(address).Get())->second);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"StarCashXaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"StarCashXaddress\"  (string, required) The StarCashX address to use for the private key.\n"
    ...
    CStarCashXcoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"StarCashXaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given StarCashXaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"StarCashXaddress\"  (string, required) The StarCashX address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount   (numeric) The total amount in STARX received at this address.\n"
            "\nExamples:\n"
    ...
    // Bitcoin address
    CStarCashXcoinAddress address = CStarCashXcoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid StarCashX address");
...
void GetSTARXuntAddresses(string strSTARXunt, set<CTxDestination>& setAddress)
{
...
Value getreceivedbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount              (numeric) The total amount in STARX received for this account.\n"
            "\nExamples:\n"
    ...
    GetSTARXuntAddresses(strSTARXunt, setAddress);
...
int64_t GetSTARXuntBalance(CWalletDB& walletdb, const string& strSTARXunt, int nMinDepth, const isminefilter& filter)
{
    int64_t nBalance = 0;
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        ...
        wtx.GetSTARXuntAmounts(strSTARXunt, nReceived, nSent, nFee, filter);
    ...
    nBalance += walletdb.GetSTARXuntCreditDebit(strSTARXunt);

    return nBalance;
}

int64_t GetSTARXuntBalance(const string& strSTARXunt, int nMinDepth, const isminefilter& filter)
{
    CWalletDB walletdb(pwalletMain->strWalletFile);
    return GetSTARXuntBalance(walletdb, strSTARXunt, nMinDepth, filter);
}
...
Value getbalance(const Array& params, bool fHelp)
{
    ...
    if (params[0].get_str() == "*") {
        ...
        for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
        {
            ...
            CAmount allFee;
            string strSentSTARXunt;
            list<pair<CTxDestination, int64_t> > listReceived;
            list<pair<CTxDestination, int64_t> > listSent;
            wtx.GetAmounts(listReceived, listSent, allFee, strSentSTARXunt, filter);
            if (wtx.GetDepthInMainChain() >= nMinDepth)
            {
    ...
    CAmount nBalance = GetSTARXuntBalance(strSTARXunt, nMinDepth, filter);
...
Value movecmd(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            ...
            "\nExamples:\n"
            "\nMove 0.01 STARX from the default account to the account named tabby\n"
            + HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 STARX timotei to akiko with a comment and funds have 10 confirmations\n"
            + HelpExampleCli("move", "\"timotei\" \"akiko\" 0.01 10 \"happy birthday!\"") +
            "\nAs a json STARX call\n"
            + HelpExampleRpc("move", "\"timotei\", \"akiko\", 0.01, 10, \"happy birthday!\"")
        );

    accountingDeprecationCheck();

    string strFrom = STARXuntFromValue(params[0]);
    string strTo = STARXuntFromValue(params[1]);
    ...
    // Debit
    CSTARXuntingEntry debit;
    debit.nOrderPos = pwalletMain->IncOrderPosNext(&walletdb);
    debit.strSTARXunt = strFrom;
    debit.nCreditDebit = -nAmount;
    debit.nTime = nNow;
    debit.strOtherSTARXunt = strTo;
    debit.strComment = strComment;
    pwalletMain->AddSTARXuntingEntry(debit, walletdb);

    // Credit
    CSTARXuntingEntry credit;
    credit.nOrderPos = pwalletMain->IncOrderPosNext(&walletdb);
    credit.strSTARXunt = strTo;
    credit.nCreditDebit = nAmount;
    credit.nTime = nNow;
    credit.strOtherSTARXunt = strFrom;
    credit.strComment = strComment;
    pwalletMain->AddSTARXuntingEntry(credit, walletdb);
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"toStarCashXaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a StarCashX address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toStarCashXaddress\"  (string, required) The StarCashX address to send funds to.\n"
            "3. amount                (numeric, required) The amount in STARX. (transaction fee is added on top).\n"
            ...
            "\"transactionid\"        (string) The transaction id.\n"
            "\nExamples:\n"
            "\nSend 0.01 STARX from the default account to the address, must have at least 1 confirmation\n"
    ...
    string strSTARXunt = STARXuntFromValue(params[0]);
    CStarCashXcoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid StarCashX address");
    CAmount nAmount = AmountFromValue(params[2]);
    ...
    CWalletTx wtx;
    wtx.strFromSTARXunt = strSTARXunt;
    ...
    // Check funds
    int64_t nBalance = GetSTARXuntBalance(strSTARXunt, nMinDepth, ISMINE_SPENDABLE);
    if (nAmount > nBalance)
        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS, "STARXunt has insufficient funds");
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The StarCashX address is the key, the numeric amount in STARX is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    set<CStarCashXcoinAddress> setAddress;
    vector<pair<CScript, int64_t> > vecSend;

    int64_t totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CStarCashXcoinAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid StarCashX address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a StarCashX address or hex-encoded public key.\n"
            "If 'account' is specified, assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of StarCashX addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) StarCashX address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"StarCashXaddress\"  (string) A StarCashX address associated with the keys.\n"
    ...
    string strSTARXunt;
    if (params.size() > 2)
        strSTARXunt = STARXuntFromValue(params[2]);
    ...
    pwalletMain->SetAddressBookName(innerID, strSTARXunt);
    return CStarCashXcoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fBySTARXunts)
{
    // Minimum confirmations
    int nMinDepth = 1;
    if (params.size() > 0)
    ...
    // Tally
    map<CStarCashXcoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    // Reply
    Array ret;
    map<string, tallyitem> mapSTARXuntTally;
    BOOST_FOREACH(const PAIRTYPE(CStarCashXcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CStarCashXcoinAddress& address = item.first;
        const string& strSTARXunt = item.second;
        map<CStarCashXcoinAddress, tallyitem>::iterator it = mapTally.find(address);
        ...
        if (fBySTARXunts)
        {
            tallyitem& item = mapSTARXuntTally[strSTARXunt];
        ...
        else
        {
            ...
            obj.push_back(Pair("account",       strSTARXunt));
    ...
    if (fBySTARXunts)
    {
        for (map<string, tallyitem>::iterator it = mapSTARXuntTally.begin(); it != mapSTARXuntTally.end(); ++it)
        {
...
Value listreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in STARX received by the address\n"
...
void ListTransactions(const CWalletTx& wtx, const string& strSTARXunt, int nMinDepth, bool fLong, Array& ret, const isminefilter& filter)
{
    CAmount nFee;
    string strSentSTARXunt;
    list<pair<CTxDestination, int64_t> > listReceived;
    ...
    wtx.GetAmounts(listReceived, listSent, nFee, strSentSTARXunt, filter);

    bool fAllSTARXunts = (strSTARXunt == string("*"));
    bool involvesWatchonly = wtx.IsFromMe(ISMINE_WATCH_ONLY);

    // Sent
    if ((!wtx.IsCoinStake()) && (!listSent.empty() || nFee != 0) && (fAllSTARXunts || strSTARXunt == strSentSTARXunt))
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& s, listSent)
        {
            ...
            entry.push_back(Pair("account", strSentSTARXunt));
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        ...
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& r, listReceived)
        {
            ...
            if (fAllSTARXunts || (account == strSTARXunt))
            {
                Object entry;
...
void AcentryToJSON(const CSTARXuntingEntry& acentry, const string& strSTARXunt, Array& ret)
{
    bool fAllSTARXunts = (strSTARXunt == string("*"));

    if (fAllSTARXunts || acentry.strSTARXunt == strSTARXunt)
    {
        Object entry;
        entry.push_back(Pair("account", acentry.strSTARXunt));
        entry.push_back(Pair("category", "move"));
        entry.push_back(Pair("time", (int64_t)acentry.nTime));
        entry.push_back(Pair("amount", ValueFromAmount(acentry.nCreditDebit)));
        entry.push_back(Pair("otheraccount", acentry.strOtherSTARXunt));
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"StarCashXaddress\",    (string) The StarCashX address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in STARX. This is negative for the 'send' category, and for the\n"
    ...
    for (CWallet::TxItems::const_reverse_iterator it = txOrdered.rbegin(); it != txOrdered.rend(); ++it)
    {
        ...
        if (pwtx != 0)
            ListTransactions(*pwtx, strSTARXunt, 0, true, ret, filter);
        CSTARXuntingEntry *const pacentry = (*it).second.second;
        if (pacentry != 0)
            AcentryToJSON(*pacentry, strSTARXunt, ret);
...
Value listaccounts(const Array& params, bool fHelp)
{
    ...
    map<string, CAmount> mapSTARXuntBalances;
    BOOST_FOREACH(const PAIRTYPE(CTxDestination, string)& entry, pwalletMain->mapAddressBook) {
        if (IsMine(*pwalletMain, entry.first) & includeWatchonly) // This address belongs to me
            mapSTARXuntBalances[entry.second] = 0;
    }
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        ...
        if (nDepth < 0)
            continue;
        wtx.GetAmounts(listReceived, listSent, nFee, strSentSTARXunt, includeWatchonly);
        mapSTARXuntBalances[strSentSTARXunt] -= nFee;
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& s, listSent)
            mapSTARXuntBalances[strSentSTARXunt] -= s.second;
        if (nDepth >= nMinDepth && wtx.GetBlocksToMaturity() == 0)
        {
            BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& r, listReceived)
                if (pwalletMain->mapAddressBook.count(r.first))
                    mapSTARXuntBalances[pwalletMain->mapAddressBook[r.first]] += r.second;
                else
                    mapSTARXuntBalances[""] += r.second;
        }
    }

    const list<CSTARXuntingEntry> & acentries = pwalletMain->laccentries;
    BOOST_FOREACH(const CSTARXuntingEntry& entry, acentries)
        mapSTARXuntBalances[entry.strSTARXunt] += entry.nCreditDebit;

    Object ret;
    BOOST_FOREACH(const PAIRTYPE(string, CAmount)& accountBalance, mapSTARXuntBalances) {
...
Value listsinceblock(const Array& params, bool fHelp)
{
    if (fHelp)
       throw runtime_error(
            ...
            "    \"address\":\"StarCashXaddress\",    (string) The StarCashX address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in STARX. This is negative for the 'send' category, and for the 'move' category for moves \n"
...
Value gettransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"StarCashXaddress\",   (string) The StarCashX address involved in the transaction\n"
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending STARX\n"
            "\nArguments:\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending StarCashX\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"StarCashXaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; StarCashX server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in STARX/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new StarCashX stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid StarCashX stealth address."));
        return result;
    };
